...
索引和lz4压缩后的索引只需要留下一个，我更偏向lz4压缩后的索引，但不知道为什么好像在这里lz4并没有成功压缩索引的大小
索引会不断增长：第1次保存57MB → 第2次保存114MB → ... → 最后保存4.9GB，你指的是索引一直在内存里，最后一次一次性保存4.9GB，还是说每隔五分钟，写入一次，我觉得之前的逻辑更正常，只是他每次调用都会覆盖

jasper的向量维度通过yaml文件控制，我建议1024而不是2048
先注释掉zstd压缩代码，暂时不需要压缩
很明显jasper build embedding的代码有问题，
