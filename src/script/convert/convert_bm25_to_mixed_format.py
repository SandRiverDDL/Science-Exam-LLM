...
将旧的全parquet格式索引转换为新的混合格式：
- tokenized_chunks → pkl
- inverted_index → pkl
- chunk_len, idf_table, metadata → parquet

使用方式：
    python convert_bm25_to_mixed_format.py <source_index_dir> <target_index_dir>

示例：
    python convert_bm25_to_mixed_format.py data/processed/bm25_index data/processed/bm25_index_mixed
"""
...
def convert_bm25_index(source_dir: str, target_dir: str):
    ...
    Args:
        source_dir: 源索引目录（旧的全parquet格式）
        target_dir: 目标索引目录（新的混合格式）
    """
    ...
    # 加载tokenized_chunks（旧格式）
    print("[2/5] 加载tokenized_chunks（从parquet）...")
    chunks_df = pd.read_parquet(source_path / 'tokenized_chunks.parquet')
    chunk_ids = chunks_df['chunk_id'].tolist()
    tokenized_chunks = [tokens.split() for tokens in chunks_df['tokens']]
    ...
    # 加载inverted_index（旧格式）
    print("[5/5] 加载inverted_index（从parquet）...")
    inverted_df = pd.read_parquet(source_path / 'inverted_index.parquet')
    inverted_index = defaultdict(list)
    for _, row in inverted_df.iterrows():
        token = row['token']
        doc_indices = [int(idx) for idx in row['doc_indices'].split(',')]
        inverted_index[token] = doc_indices
    ...
    # 保存tokenized_chunks（pkl）
    print("[1/5] 保存tokenized_chunks.pkl...")
    with open(target_path / 'tokenized_chunks.pkl', 'wb') as f:
        pickle.dump({
            'chunk_ids': chunk_ids,
            'tokenized_chunks': tokenized_chunks
        }, f, protocol=pickle.HIGHEST_PROTOCOL)
    
    # 保存inverted_index（pkl）
    print("[2/5] 保存inverted_index.pkl...")
    with open(target_path / 'inverted_index.pkl', 'wb') as f:
        pickle.dump(dict(inverted_index), f, protocol=pickle.HIGHEST_PROTOCOL)
    
    # 保存chunk_len（parquet）
    print("[3/5] 保存chunk_len.parquet...")
    chunk_len_data = {
        'chunk_id': chunk_ids,
        'chunk_len': chunk_len
    }
    ...
    print()
    print("文件清单:")
    print("  ✓ tokenized_chunks.pkl  (pkl)")
    print("  ✓ inverted_index.pkl    (pkl)")
    print("  ✓ chunk_len.parquet     (parquet)")
    print("  ✓ idf_table.parquet     (parquet)")
    print("  ✓ metadata.parquet      (parquet)")
